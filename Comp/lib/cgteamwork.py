...
def addMenu():
    m = nuke.menu("Nuke")
    m = m.addMenu(u'CGTeamWork_合成'.encode('UTF-8'))
    m.addCommand(u'设置工程'.encode('UTF-8'),"cgteamwork.CGTeamWork.ask_database()")
    m.addCommand(u'添加note'.encode('UTF-8'),"cgteamwork.Shot().ask_add_note()")
    m.addCommand(u'上传nk文件'.encode('UTF-8'),"cgteamwork.Shot().upload_nk_file()")
