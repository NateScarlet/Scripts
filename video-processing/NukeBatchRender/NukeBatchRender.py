...
class Config(dict):
    ...
    path = os.path.join(os.getenv('UserProfile'), '.NukeBatchRender.json')
...
class SingleInstance(object):
    def __init__(self):
        PID = Config()['PID']
        if isinstance(PID, int) and self.is_pid_exists(PID) is True:
    ...
    def unlock_file(self, file):
        ...
        if os.path.isfile(_unlocked_name):
            ...
            try:
                ...
                print_(u'解锁: {}'.format(file)) 
                self._logger.info(u'解锁: {}'.format(file))
                return _unlocked_name
            except WindowsError:
                print_(u'**错误** 其他程序占用文件: {}'.format(file))
