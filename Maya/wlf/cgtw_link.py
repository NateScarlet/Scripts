...
class AssetsLink(object):
    ...
    def get_shot_task_id(self):
        ...
        if not initiated:
            print(u'找不到对应流程: {}'.format(self.config['PIPELINE']))
            return False
    ...
    def get_assets(self):
        for i in self._config['NAMESPACES']:
            ...
            asset_name = re.sub(r'(_Lo$|_Hi$)', '', asset_name)
            initiated = self._assets.init_with_filter([['asset.asset_name', '=', asset_name]])
            if not initiated:
                print(u'未找到资产: {} -> {}'.format(i, asset_name).encode(SYS_CODEC))
                continue
            print(u'找到资产: {} -> {}'.format(i, asset_name).encode(SYS_CODEC))
    ...
    def link_asset(self):
        ...
        print(u'\n{num}个资产Link至镜头: {shot}  '.format(shot=self._config['SHOT'], num=len(self._config['asset_id_list'])).encode(SYS_CODEC))


def main(tempfile=None):
    config =None
