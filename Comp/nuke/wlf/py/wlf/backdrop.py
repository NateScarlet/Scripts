...
def create_backdrop():
    customPreset = None
    sep = '"'
    presets = ['Elements', 'Output', 'Roto', 'Camera_Projection', 'Camera_Setup', 'CG', 'Color_Correction', 'Despill',
               'Edge_Fixes', 'FX', 'Key', 'Matte', 'Lens_Flare', 'Light_Setup', 'Relight', 'Stereo_Fixes', 'Temp', 'Test']
    ...
    if customPreset is not None:
        ...
        if customLabel == '':
            n = nuke.nodes.BackdropNode(xpos=xMinMaxPos[0] - 10, bdwidth=xMinMaxPos[1] - xMinMaxPos[0] + 110, ypos=yMinMaxPos[0] - 85,
                                        bdheight=yMinMaxPos[1] - yMinMaxPos[0] + 160, label='<center>' + icon + presetLabel, tile_color=hexColour, note_font_size=30)
        else:
            n = nuke.nodes.BackdropNode(xpos=xMinMaxPos[0] - 10, bdwidth=xMinMaxPos[1] - xMinMaxPos[0] + 110, ypos=yMinMaxPos[0] - 85,
                                        bdheight=yMinMaxPos[1] - yMinMaxPos[0] + 160, label='<center>' + icon + customLabel, tile_color=hexColour, note_font_size=30)
