...
def getMinMax(srcNode, channel='depth.Z'):
    ...
    '''
    MinColor = nuke.nodes.MinColor(
        channels=channel, target=0, inputs=[srcNode])
    ...
    if ok:
        ...
        for i in nuke.allNodes('Read'):
            ...
            if file_path.startswith(render_path):
                ...
                file_path = re.sub(
                    r'\.([\d#]+)\.', lambda matchobj: r'.%0{}d.'.format(len(matchobj.group(1))), file_path)
